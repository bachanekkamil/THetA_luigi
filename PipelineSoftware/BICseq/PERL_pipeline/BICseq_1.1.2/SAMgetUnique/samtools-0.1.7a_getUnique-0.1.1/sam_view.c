...
int main_samview(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "i:ed:Sbt:hHo:q:f:F:ul:r:xX?T:CU:")) >= 0) {
		switch (c) {
	...
	if (argc == optind + 1) { // convert/print the entire file
		...
		while ((r = samread(in, b)) >= 0) { // read one alignment from `in'
			if (!__g_skip_aln(in->header, b)) {
				...
				if(args_unique==NULL) samwrite(out, b); // write the alignment to `out'
				else {report_unique_read(in->header,b);}
				//samwrite(out, b);
			}
		}
